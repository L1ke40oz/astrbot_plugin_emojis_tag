...
on_llm_response  → 解析 <情绪> 标签，为每个标签随机选图；
                   · completion_text（决定发送内容）只保留剥离标签后的干净正文，不含摘要
                   · 摘要仅追加写入对话历史（run_context），供模型记忆
on_decorating_result → 从消息链剥离残留标签，并把表情包图片随机穿插进句子之间
发送给用户       → 干净文本 + 随机穿插的表情包图片，无标签、无摘要
```

> 摘要（如 `[发送了表情包：happy-xxx.jpg]`）**只进对话历史，绝不发送给用户**。
> 这样即使模型只发表情包、不说话，也不会把摘要当成正文发出去。
...
png/jpg 原图分辨率可能很大，导致聊天里图片显示得很大。开启「图片压缩」后，发送前会把
图片**最长边等比缩小**到上限（默认 512px），不超过上限则原样发送。

- 需要 [Pillow](https://pypi.org/project/pillow/)（`pip install pillow`，已写入 `requirements.txt`）；
  未安装时自动跳过压缩、发送原图。
- 仅压缩静态图 `jpg/jpeg/png/bmp`；`gif/webp` 可能是动图，缩放会破坏动画，故跳过。
- 压缩只影响**发送出去的图片**，不改动原文件，也不影响历史摘要里记录的文件名。
- 压缩结果缓存在插件目录的 `.resized_cache/`（按原图修改时间+尺寸+质量命名），命中即复用。
...
| 发送图片压缩 → 启用图片压缩 | `true` | 关闭则始终发送原图 |
| 发送图片压缩 → 最长边像素上限 | `512` | 超过则等比缩小到该值 |
| 发送图片压缩 → JPEG 压缩质量 | `85` | 1-100，仅对 jpg/jpeg 生效 |
